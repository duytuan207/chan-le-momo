...
if ($_POST['xoa'] == 'xoa') {
    ...
    if ($id >= 1) {
        $dbh->prepare("DELETE FROM `my_viettel_tkc` WHERE `id` = ?")->execute(array($id));
    ...
    header('location: /admin/myviettel_tkc.php');
    exit;
}else{
...
if (isset($_POST['id'])) {
    ...
    $check_error = $dbh->query("SELECT * FROM my_viettel_tkc where username = '{$username}'")->fetchAll();
    if ($check_error == true) {
    ...
    $dbh->prepare("INSERT INTO my_viettel_tkc (username, password, khoa) VALUES (?,?,?)")
        ->execute(array($username, $password, $khoa));
        $_SESSION['admin_message'] = 'Thêm myviettel_tkc mới thành công';
        $_SESSION['success'] = 'ok';
        header('Location: /admin/myviettel_tkc.php');
        exit;
