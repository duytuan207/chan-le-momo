...
if(isset($_POST['duyet'])){
    ...
    $info =  db_row("SELECT * FROM history_bank WHERE id = '".(int)$_POST['duyet']."' AND status ='0' LIMIT 1"); 
    if($info){
    // dòng này thêm lịch sử chính

        $text ='Rút tiền thành công';
        db_insert("log", array('email' => $info['email'], 'type' => 'Rút Tiền','mota' => $text,'status' => 'Thành Công','amount' => '-'.$_POST['cash_real'] , 'time' => time())); // log
      //  db_update("history_bank", array("text" => $_POST['lydo'], "cash_real" => $_POST['cash_real'], "cash" => $_POST['cash'], "status" => 2), "id = '".$_POST['duyet']."'"); // cập nhật trạng thái   
        db_update("history_bank", array("status" => 2), "id = '".(int)$_POST['duyet']."'"); // cập nhật trạng thái
    }    

        load_url(setting('domain')."/admin/withdrawal.php");
    // end
    
    }
    $info =  db_row("SELECT * FROM history_bank WHERE id = '".$_POST['duyet']."' LIMIT 1"); 
?>
                                <div class="card">

                                                                <div class="card-block">
                                                      <form action="" method="post">
                                                                    <h4 class="sub-title">Duyệt Lệnh Rút</h4>
...
<?php
    ...
    if(isset($_POST['xacnhanhuy'])){ 
        $info =  db_row("SELECT * FROM history_bank WHERE id = '".$_POST['huy']."' and status ='0' LIMIT 1"); 
        if($info){
        $text =' Hoàn Tiền Rút Về Ngân Hàng';
        db_insert("log", array('email' => $info['email'], 'type' => 'Rút Tiền','mota' => $text,'status' => 'Thất Bại','amount' => (int)$info['amount'] , 'time' => time())); // log
        
        db_update("client", array("cash" => (client($info['fid'])["cash"] + (int)$info['amount'])), "email = '".$info['email']."'");

        db_update("history_bank", array("status" => 1), "id = '".$_POST['huy']."'"); // cập nhật trạng thái
        }
        load_url(setting('domain')."/admin/withdrawal.php");
    }
?>                           <div class="card">
                                                                ...
                                                                <div class="card-block">
                                                      <form action="" method="post">
                                                                    <h4 class="sub-title">Hủy Lệnh Rút</h4>
...
<div class="pcoded-inner-content">
                            <div class="main-body">
                                ...
                                <h4 class="title">Danh sách Rút Tiền</h4>
                                            </div>
                                            <div class="card-block">
...
if(!$_POST){
$result = db_row("SELECT count(id) AS counter FROM history_bank WHERE status = '0'");
}else{
$result = db_row("SELECT count(id) AS counter FROM history_bank WHERE $where");
}
// Phân trang ở đây
...
if(!$_POST){
$list_client = db_list("SELECT * FROM history_bank WHERE status = '0' ORDER BY time ASC LIMIT {$startpoint} , {$per_page}");
}else{
$list_client = db_list("SELECT * FROM history_bank where $where ORDER BY status ASC, time DESC LIMIT 40");
}
...
?>
                                            ...
                                            <td>
                                                ...
                                                <form action="" method="post">
                                                <button type="submit" class=" btn-primary btn-xs" title="Duyệt" name="duyet" style="padding: 6px 12px;"  value="<?php echo $item["id"]; ?>">Duyệt</button>
...
?>                                         
                                    ...
                                    <hr>      <?php
                                            if(!$_POST){
                                    echo pagination_html($result['counter'],$per_page,$page,$url='/admin/withdrawal.php?page=');
                                            }
                                    ?>
 ...
 <?php
    $delay = db_row("SELECT SUM(cash_real) AS total FROM history_bank WHERE status = '0'");
?>
                          <div class="card-header">
